...
# Support multiple WeChat containers
CONTAINERS = {
...
def main():
    parser = argparse.ArgumentParser(description="Extract WeChat database keys via memory scanning")
    parser.add_argument(
        "--account", "-a", choices=["1", "2"], default="1",
        help="Which WeChat account: 1=xinWeChat, 2=xinWeChat2 (default: 1)"
    )
    parser.add_argument(
        "--pid", "-p", type=int, default=None,
        help="Specific WeChat PID to attach to (optional, auto-detect if not set)"
    )
    parser.add_argument(
    ...
    OUTPUT_FILE = args.output or f"wechat_keys_{args.account}.json"

    print("=" * 60)
    print(f"  WeChat Memory Scanner - Account {args.account} ({container})")
    print("=" * 60)
    ...
    if args.pid:
        process = target.AttachToProcessWithID(
            debugger.GetListener(), args.pid, error
        )
    else:
    ...
    if not error.Success():
        ...
        print("[!] Tip: use --pid <PID> to specify which WeChat process.")
        print("[!]   Find PIDs: pgrep -l WeChat")
        sys.exit(1)
